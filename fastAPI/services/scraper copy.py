...
async def schedule_manual_run(config: ScrapeConfig) -> None:
    ...
    async with _lock:
        ...
        # Run _execute_manual_run in a separate thread to prevent blocking the event loop
        task = asyncio.create_task(asyncio.to_thread(_execute_manual_run, config_data), name=f"scrape-config-{config_id}")
...
async def _execute_manual_run(config_data: Dict[str, Any]) -> None:
    ...
    try:
        # await init_db()
        logger.info(f"DEBUG: init_db skipped for config {config_id}")
        ...
        # )
        logger.info(f"DEBUG: _update_status (running) skipped for config {config_id}")

        # count_before = await _count_listings()
        logger.info(f"DEBUG: _count_listings skipped for config {config_id}")
        ...
        return_code = await asyncio.to_thread(process.wait) # Wrap blocking call in to_thread
        ...
        if return_code == 0:
            ...
            # )
            logger.info(f"DEBUG: Scrape for config {config_id} completed successfully (status update skipped)")
        else:
            # await _update_status(
            #     config_id,
            #     "failed",
            ...
            # )
            logger.info(f"DEBUG: Scrape for config {config_id} exited with code {return_code} (status update skipped)")
        ...
        # )
        logger.info(f"DEBUG: Scrape for config {config_id} failed with exception (status update skipped)")
        raise
